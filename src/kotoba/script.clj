...
(defn- infer-call-type [op args env signatures]
  (let [types (mapv #(infer-type % env signatures) args)]
    (cond
      ...
      (= op 'document-symbol)
      (do (require-arity! op args 1) (require-type! (first types) :symbol (first args)) :document)
      (= op 'document-vector)
      ...
      (do (doseq [[arg type] (map vector args types)] (require-type! type :document arg))
          (when (> (count args) max-document-container-items)
            (fail! "KIR document list exceeds item limit" {:items (count args)}))
          :document)
      (= op 'document-set)
      (do (doseq [[arg type] (map vector args types)] (require-type! type :document arg))
          (when (> (count args) max-document-container-items)
            (fail! "KIR document set exceeds item limit" {:items (count args)}))
          :document)
      (= op 'document-map)
...
(defn- emit-call [op args env functions counter]
  (let [a #(emit-expr % env functions counter)]
    (cond
      ...
      (= op 'document-symbol) (str "makeDocScalar('symbol'," (a (first args)) ")")
      (= op 'document-vector) (str "makeDocVector([" (str/join "," (map a args)) "])")
      (= op 'document-list) (str "makeDocList([" (str/join "," (map a args)) "])")
      (= op 'document-set) (str "makeDocSet([" (str/join "," (map a args)) "])")
      (= op 'document-map)
      (str "makeDocMap(["
      ...
      (= op 'document-set-contains?) (str "docSetContains(" (a (first args)) "," (a (second args)) ")")
      (= op 'document-vector-at) (str "docVectorAt(" (a (first args)) "," (a (second args)) ")")
      (= op 'document-list-at) (str "docListAt(" (a (first args)) "," (a (second args)) ")")
      ...
      (= op 'document-symbol-value) (str "docScalarOption('symbol'," (a (first args)) ")")
...
(defn emit
  ...
  (let [function-names (mapv :name (:functions kir))
        ...
        (str "export const kotobaArtifact=Object.freeze({schema:'" artifact-schema
             ...
             "const valueEqual=(a,b)=>{if(Array.isArray(a)!==Array.isArray(b))return false;"
             "if(Array.isArray(a)){"
             ...
             "if(++state.nodes>" max-document-nodes ")throw new Error('doc-node-limit');"
             "if(state.seen.has(x))throw new Error('doc-shared-or-cyclic');state.seen.add(x);"
             "const tag=x[0];"
             "if(tag==='null'){if(x.length!==1)throw new Error('invalid-doc-null');return Object.freeze(['null']);}"
             ...
             "if(tag==='string'||tag==='keyword'||tag==='symbol'){if(x.length!==2)throw new Error('invalid-doc-text');"
             "const s=tag==='string'?assertString(x[1]):tag==='keyword'?assertKeyword(x[1]):assertSymbol(x[1]);state.bytes+=utf8Bytes(s);"
             "if(state.bytes>" max-document-utf8-bytes ")throw new Error('doc-utf8-limit');return Object.freeze([tag,s]);}"
             "if(tag==='vector'||tag==='list'){if(x.length!==2||!Array.isArray(x[1])||x[1].length>" max-document-container-items
             ")throw new Error('invalid-doc-sequence');return Object.freeze([tag,Object.freeze(x[1].map(y=>walk(y,d+1)))]);}"
             "if(tag==='set'){if(x.length!==2||!Array.isArray(x[1])||x[1].length>" max-document-container-items
             ")throw new Error('invalid-doc-set');const items=x[1].map(y=>walk(y,d+1));"
             "for(let i=1;i<items.length;i++)if(docCompare(items[i-1],items[i])>=0)throw new Error('noncanonical-doc-set');"
             "return Object.freeze([tag,Object.freeze(items)]);}"
             ...
             "let key;if(typeof e[0]==='string'){const s=assertKeyword(e[0]);state.bytes+=utf8Bytes(s);key=Object.freeze(['keyword',s]);}"
             "else if(Array.isArray(e[0])&&e[0].length===2&&e[0][0]==='keyword'){if(state.seen.has(e[0]))throw new Error('doc-shared-or-cyclic');state.seen.add(e[0]);const s=assertKeyword(e[0][1]);state.bytes+=utf8Bytes(s);key=Object.freeze(['keyword',s]);}else key=walk(e[0],d+1);"
             "if(state.bytes>" max-document-utf8-bytes ")throw new Error('doc-utf8-limit');"
             ...
             "const makeDocVector=items=>assertDoc(['vector',items.map(item=>assertDoc(item))]);"
             "const makeDocList=items=>assertDoc(['list',items.map(item=>assertDoc(item))]);"
             "const makeDocSet=items=>{if(!Array.isArray(items)||items.length>" max-document-container-items
             ")throw new Error('invalid-doc-set');const sorted=items.map(assertDoc).sort(docCompare);"
             "for(let i=1;i<sorted.length;i++)if(docCompare(sorted[i-1],sorted[i])===0)throw new Error('duplicate-doc-set-item');"
             ...
             "const docListEntries=v=>{v=assertDoc(v);if(v[0]!=='list')throw new Error('doc-list-required');return v[1];};"
             "const docSetEntries=v=>{v=assertDoc(v);if(v[0]!=='set')throw new Error('doc-set-required');return v[1];};"
             ...
             "if(s.length%2)throw new Error('document-read-hex-odd');"
             "if(s.length>1048576)throw new Error('document-read-hex-limit');"
             "if(!/^[0-9a-f]*$/.test(s))throw new Error('document-read-hex-charset');"
             "const out=new Uint8Array(s.length/2);"
             "for(let i=0;i<out.length;i++)out[i]=parseInt(s.substr(i*2,2),16);"
             "return out;};"
             "const docFromCanonicalBytes=bytes=>{let i=0;const len=bytes.length;"
             "const take=()=>{if(i>=len)throw new Error('document-read-truncated');return bytes[i++];};"
             "const peek=()=>{if(i>=len)throw new Error('document-read-truncated');return bytes[i];};"
             "const takeUntil=sep=>{const start=i;while(i<len&&bytes[i]!==sep)i++;"
             "if(i>=len)throw new Error('document-read-missing-term');"
             "const s=new TextDecoder('utf-8',{fatal:true}).decode(bytes.subarray(start,i));i++;return s;};"
             "const takeLenStr=()=>{const n=Number(takeUntil(58));"
             "if(!Number.isInteger(n)||n<0||n>65536)throw new Error('document-read-len');"
             "if(i+n>len)throw new Error('document-read-trunc-str');"
             "const s=new TextDecoder('utf-8',{fatal:true}).decode(bytes.subarray(i,i+n));i+=n;return s;};"
             "const walk=()=>{const tag=take();"
             "if(tag===110)return Object.freeze(['null']);"
             "if(tag===98){const b=take();if(b===116)return Object.freeze(['bool',true]);"
             "if(b===102)return Object.freeze(['bool',false]);throw new Error('document-read-bool');}"
             "if(tag===105){const s=takeUntil(59);return Object.freeze(['i64',BigInt(s)]);}"
             "if(tag===102){const bits=BigInt(takeUntil(59));const buf=new ArrayBuffer(8);const dv=new DataView(buf);"
             "dv.setBigInt64(0,bits,true);return Object.freeze(['f64',dv.getFloat64(0,true)]);}"
             "if(tag===115)return Object.freeze(['string',takeLenStr()]);"
             "if(tag===107){const k=takeLenStr();if(!k.startsWith(':'))throw new Error('document-read-kw');"
             "return Object.freeze(['keyword',k]);}"
             "if(tag===121)return Object.freeze(['symbol',assertSymbol(takeLenStr())]);"
             "if(tag===118){const n=Number(takeUntil(58));if(!Number.isInteger(n)||n<0||n>32)throw new Error('document-read-vc');"
             "const items=[];for(let k=0;k<n;k++)items.push(walk());return Object.freeze(['vector',Object.freeze(items)]);}"
             "if(tag===108){const n=Number(takeUntil(58));if(!Number.isInteger(n)||n<0||n>32)throw new Error('document-read-lc');"
             "const items=[];for(let k=0;k<n;k++)items.push(walk());return Object.freeze(['list',Object.freeze(items)]);}"
             ...
             "const docPrint=v=>{const hex=bytesToHex(docCanonicalBytes(assertDoc(v)));"
             ...
             "const docEdnSymbol=s=>{s=assertSymbol(s);if(s==='nil'||s==='true'||s==='false'||s[0]===':'||s[0]==='#'||"
             "/[\\s,\\[\\]{}()\"';`~^\\\\]/u.test(s)||/^[+-]?[0-9]+$/.test(s)||/^[+-]?(?:(?:[0-9]+\\.[0-9]*)|(?:[0-9]*\\.[0-9]+)|(?:[0-9]+[eE][+-]?[0-9]+))(?:[eE][+-]?[0-9]+)?$/.test(s))throw new Error('document-edn-ambiguous-symbol');return s;};"
             ...
             "throw new Error('invalid-xml-declaration');i=e+2;}comments();"
             "if(s.startsWith('<!DOCTYPE',i)){const e=s.indexOf('>',i+9);if(e<0||!/^<!DOCTYPE[ \\t\\r\\n]+html[ \\t\\r\\n]*>$/u.test(s.slice(i,e+1)))"
             "throw new Error('invalid-xml-doctype');i=e+1;}comments();element(1,'');comments();skip();"
