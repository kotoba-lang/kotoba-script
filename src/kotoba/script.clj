...
(defn emit
  ...
  (let [function-names (mapv :name (:functions kir))
        ...
        (str "export const kotobaArtifact=Object.freeze({schema:'" artifact-schema
             ...
             "const docCount=v=>{v=assertDoc(v);if(v[0]!=='map'&&v[0]!=='vector'&&v[0]!=='list')throw new Error('doc-container-required');return BigInt(v[1].length);};"
             ...
             "if(c==='#')fail('dispatch');if(c===')'||c===']'||c==='}')fail('closing');"
             ...
             "const docEqual=(a,b)=>{a=assertDoc(a);b=assertDoc(b);const eq=(x,y)=>{if(x[0]!==y[0])return false;const t=x[0];if(t==='null')return true;if(t!=='vector'&&t!=='list'&&t!=='map')return x[1]===y[1];if(x[1].length!==y[1].length)return false;if(t==='vector'||t==='list'){for(let i=0;i<x[1].length;i++)if(!eq(x[1][i],y[1][i]))return false;return true;}for(let i=0;i<x[1].length;i++)if(x[1][i][0]!==y[1][i][0]||!eq(x[1][i][1],y[1][i][1]))return false;return true;};return eq(a,b);};"
